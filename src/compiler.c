    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            ...
            if (current->function->arity > 255) {
                errorAtCurrent("Can't have more than 255 parametres.");
            }
            uint8_t constant = parseVariable("Expect parametre name.");
            defineVariable(constant);
        } while(match(TOKEN_COMMA));
    }
    consume(TOKEN_RIGHT_PAREN, "Expect ')' after parametres.");
...
static void returnStatement() {
    ...
    if (match(TOKEN_SEMICOLON)) {
        ...
        if (current->type == TYPE_INITIALISER) {
            error("Can't return a value from the initialiser.");
        }
